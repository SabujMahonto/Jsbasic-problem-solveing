// ২. তুমি কি জানো কখন let দিয়ে ভেরিয়েবল লিখতে হয়ে আর কখন const দিয়ে ভেরিয়েবল লিখতে হয়। যদি লিখতে পারো তাহলে লিখে ফেলো। এবং যেই যেটা দিয়ে ভেরিয়েবল লিখলে আবার সেটা চেইঞ্জ করা যায়। তাহলে যেটা দিয়ে ভেরিয়েবল ডিক্লেয়ার করলে চেইঞ্জ করা যায়। সেটার মান চেইঞ্জ করে নাও । অর্থাৎ জাস্ট দুইটা ভেরিয়েবল ডিক্লেয়ার করবে let এবং const দিয়ে। তারপর যেকোন একটার ভ্যালু চেইঞ্জ করবে। এই let এবং const মাথা গরম করে দিলে টেনশন নিয়ে না। সামনের মাইলস্টোন এর পরের মাইলস্টোন এ এইটা নিয়ে আরো ডিসকাস করা হবে। 

// 2. Do you know when to write a variable with let and when to write a variable with const? If you can write, write. And whatever you write the variable with, it can be changed again. Then with which the variable can be declared and changed. Change its value. That is, just declare two variables with let and const. Then any one will change the value. This let and const do not cause tension if the head is heated. This will be further discussed in the next milestone of the front milestone.

// const 

const myName = "sabuj mahonto"

// let 

let bookPrice = 300;
    bookPrice = 500;

    console.log(myName, bookPrice);